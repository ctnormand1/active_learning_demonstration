...
class Experiment():
    ...
    def run(self, n_jobs=1):
        ...
        pbars = [tqdm(position=i) for i in range(n_jobs)]
        while readers:
...
def run_trial(trial, data, session, write_pipe, model_func, pbar_num):
    ...
    n_annotation_batches = round(n_train / trial.config.batch_size + 0.5)
    ...
    for i in range(1, 1 + n_annotation_batches):
        ...
        h = model.fit(X_train[is_annotated], y_train[is_annotated],
            validation_data=(X_val, y_val), batch_size=64, epochs=50, verbose=0,
